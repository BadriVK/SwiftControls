...
func MB_TEXTSIZE(text: String?, font: UIFont) -> CGSize {
    return (text != nil && (text!).characters.count > 0) ? (text! as NSString).sizeWithAttributes([NSFontAttributeName: font]) : CGSizeZero
}

func MB_MULTILINE_TEXTSIZE(text: String?, font: UIFont, maxSize: CGSize, mode: NSLineBreakMode) -> CGSize {
    return (text != nil && (text!).characters.count > 0) ? (text! as NSString).boundingRectWithSize(maxSize, options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes: [NSFontAttributeName: font], context: nil).size : CGSizeZero
}
